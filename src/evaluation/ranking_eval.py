"""
Evaluate the rankings generated by sentence similarity facetid_models.
"""
import sys
import os
import errno
...
def graded_eval_pool_rerank_unf(data_path, run_path, method, dataset, split):
    """
    ...
    perq_csv.writeheader()
    print('Precision and recall at rank: {:}'.format(ATKS))
    wandb.init(project="aspire", job_type="evaluation", name=f"{method}_{dataset}_{split}")
    wandb.config.update({"method": method, "dataset": dataset, "split": split})
    ...
    wandb.log({f"aggregated_metrics/{dataset}_{split}": aggmetrics})
    wandb.finish()
