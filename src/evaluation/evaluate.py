...
def evaluate(results_dir: str,
    ...
    for facet_i in metrics.facet.unique():
        for split in metrics.split.unique():
            ...
            logging.info('\n'.join([f'{k}\t{agg_results[k]}' for k in ('mean_av_precision', 'ndcg%20')]))
...
def main(args):
    ...
    # init results dir
    # results_dir = get_results_dir(args.results_dir, args.dataset_name, args.model_name, datetime.now().strftime("%Y-%m-%d"))
    ...
    if 'encode' in args.actions or 'score' in args.actions:
        ...
        model = get_model(model_name=args.model_name, trained_model_path=args.trained_model_path)
    ...
    # task_description = 'Retrieve semantically similar scientific papers.' if args.instruct else None
    ...
    if 'score' in args.actions:
        ...
        if args.facet == 'all':
            for facet in FACETS:
                score(model, dataset, facet=facet, scores_filename=get_scores_filename(results_dir, facet=facet), query_instruct=args.query_instruct)
        else:
            score(model, dataset, facet=args.facet, scores_filename=get_scores_filename(results_dir, facet=args.facet), query_instruct=args.query_instruct)
